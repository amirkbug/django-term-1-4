...
class Services(models.Model):
    ...
    content = models.TextField(default="salam")
    specials = models.ManyToManyField(Special_services , null=True , blank=True)
    description = models.TextField(default="text desc")
    category = models.ManyToManyField(Category)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    catalog_file = models.TextField(max_length=250 , default="x")
    catalog_doc = models.TextField(max_length=250 , default="x")
